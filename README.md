...
## Установка и быстрые команды v0.4 — шаг 2

[Пошаговая установка для новичков](INSTALL.md): `install.py` поддерживает локальную установку, обновление, doctor и удаление с предварительным просмотром, подтверждением и резервной копией. Матрешка не обязательна.

Codex: `$context-optimizer adopt`. Claude Code / Cursor / Antigravity: `/context-optimizer adopt`.
Другие режимы: `help`, `doctor`, `start`, `resume`, `check`, `status`, `optimize`, `remember`, `finish`.

Установленный навык использует `scripts/quick.py`; путь выбирается относительно прочитанного SKILL.md, не cwd.
По просьбе пользователя быстрые проверки сохраняют отчёт; `--read-only` оставляет проект без нового состояния.
`optimize` — предложения, `remember` — отдельно подтверждаемая память. Универсальное применение исправлений остаётся следующим этапом усиления.
...
подтверждения содержимого. Во втором шаге добавлены установщик и обработчик быстрых режимов
для четырёх приложений. Проверка меню каждого настоящего интерфейса и усиление универсального
применения изменений ещё не завершены; весь пакет остаётся предварительной v0.4.
